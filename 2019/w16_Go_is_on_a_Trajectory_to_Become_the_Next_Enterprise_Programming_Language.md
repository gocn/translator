...
Go 的运行速度与 C 语言差不多快，不像 JIT 语言(Java、JavaScript、Python等)，Go 二进制文件不需要启动或预热时间，因为它们以编译好且完全经过优化的本机代码形式进行传输。Go 垃圾收集器引入了[微秒](https://twitter.com/brianhatfield/status/80435581080751104)级别的暂停机制，几乎可以忽略不记。在其高效的单核性能基础之上，Go能够利用所有CPU核心[简单](https://tour.golang.org/concurrency/1)。
