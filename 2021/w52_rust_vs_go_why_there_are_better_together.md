...
虽然有一些人可能会将 Rust 和 Go 视为具有竞争力的编程语言，但 Rust 和 Go 团队都不这么认为。恰恰相反，我们的团队非常尊重其他人正在做的事情，并将这些语言视为对整个软件开发行业现代化的共同愿景的补充。
...
在本文中，我们将讨论 Rust 和 Go 的利弊以及他们如何补充和支持彼此，以及我们对每种语言最合适的建议。
...
一些公司已经发现同时使用这两种语句具有互补价值。要从我们的意见转向真实的用户体验，我们 [dropbox](https://www.dropbox.com/), [fastly](https://www.fastly.com/) 和 [Cloudflare](https://www.cloudflare.com/) 一起发表了关于 Rust 和 Go 一起使用的经验。在本文中将使用引号，以进一步的观点。
...
| 开发者接受率                 | 8.8% (#12)                                                   | 5.1% (#19)                                                   |
| 开发这最喜爱的编程语言排行榜 | 62.3% (#5)                                                   | 86.1% (#1)                                                   |
| 开发者最想学习的语言排行榜   | 17.9% (#3)                                                   | 14.6% (#5)                                                   |
...
Go 和 Rust 有很多共同点。这两种语言都是现代编程语言，他们的诞生都是为了解决软件的安全性和可伸缩性。这两种语言的创建都是为了解决当前行业中现有语言的缺陷，特别是开发者的生产率、软件的可伸缩性、安全性和并发性方面的不足。
...
现在流行的编程语言大多是在 30 多年前设计的。当这些语言被设计出来的时候，与现在有五个关键的区别:
...
相比之下，Rust 和 Go 都是为满足当今世界的开发环境来编写的，通常采用类似的方法来设计一种满足当今开发需求的语言。
...
"使用Go，“MercadoLibre” 可以将用于这项服务的服务器数量减少到原来的八分之一(从32台服务器减少到4台)，而且每个服务器可以使用更少的电力(原来是4个CPU核，现在减少到2个CPU核)。使用Go，该公司减少了88%的服务器费用，并将剩余服务器的CPU削减了一半，从而极大地节省了成本" -- [MercadoLibre Grows with Go](https://go.dev/solutions/mercadolibre/)
...
"在我们严格管理的环境中，我们运行Go代码，我们看到CPU减少了大约10%(与c++相比)，代码更整洁、更可维护。"— [Bala Natarajan, Paypal](https://go.dev/solutions/paypal/)
...
我们看到我们的速度惊人地增长了1200-1500% !我们从Scala发布模式的300-450ms，实现了更少的解析规则，到Rust 25-30ms，实现了更多的解析规则![Josh Hannaford, IBM](https://developer.ibm.com/technologies/web-development/articles/why-webassembly-and-rust-together-improve-nodejs-performance/)_
...
当今的软件开发是由不断增长和扩充的团队构建的，通常使用源代码控制以分布式的方式进行协作。Go和Rust都是为团队的工作方式而设计的，通过消除不必要的问题，如格式、安全性和复杂的组织，来改善代码审查。这两种语言都需要相对较少的上下文来理解代码正在做什么，从而允许审查人员更快地处理其他人编写的代码，并审查团队成员和团队外开源开发人员贡献的代码。
...
"在我早期的职业生涯中，我有Java和Ruby的背景，所以编写Go和Rust的代码感觉就像卸下了肩上的重担。当我在谷歌的时候，遇到一个用Go编写的服务让我松了一口气，因为我知道它很容易构建和运行。这也适用于Rust，尽管我只在一个小得多的规模上做过。我希望无限可配置构建系统的日子已经一去不复返，所有的语言都有自己的专用构建工具，这些工具可以开箱即用。-"[Sam Rose, CV Partner](https://bitfieldconsulting.com/golang/rust-vs-go)_
...
"我倾向于松一口气 Go 服务,因为它有一个非常简单,容易推断,静态类型系统和动态语言相比,并发性是一等公民, Go 的标准库是相当地优雅和强大, 这已经到点子上了。使用一个标准的Go安装，添加一个grpc库和一个数据库连接器，您只需要很少的其他东西就可以在服务器端构建任何东西，并且每个工程师都能够阅读代码并理解这些库。当用Rust编写一个模块时，Dropbox的工程师感受到了Rust的成长之痛" - Daniel Reiter Horn, Dropbox_
...
今天，普通软件项目所使用的依赖项的数量是惊人的。几十年来的软件重用目标已经在现代开发中实现了，今天的软件是使用100个项目构建的。为了做到这一点，开发人员使用软件存储库，软件存储库已经逐渐成为跨越越来越广泛的应用程序的软件开发的主要内容。开发人员所包含的每个包都有自己的依赖项。当今编程环境的语言需要轻松地处理这种复杂的依赖。
...
Go和Rust都很好地解决了当今应用程序的安全问题，它们确保用这些语言构建的代码在运行时不会暴露给用户各种经典的安全漏洞，如缓冲区溢出、释放后可重用等。通过消除这些顾虑，开发者可以更加专注于自己的业务问题，并构建默认情况下更安全的应用程序。
...
"当您处理得到的错误时，[Rust]编译器会帮助您。这使您可以专注于业务，而不是寻找bug或处理棘手的问题。"— [Josh Hannaford, IBM](https://developer.ibm.com/technologies/web-development/articles/why-webassembly-and-rust-together-improve-nodejs-performance/)_
...
"当我们在Dropbox中构建用于存储块数据的Brotli压缩库时，我们将自己限制在Rust的安全子集，以及核心库(no-stdlib)，并将分配器指定为通用的。以这种方式使用Rust的子集，可以很容易地在客户端从Rust调用Rust- brotli库，并在服务器端使用Python和Go的C FFI。这种编译模式还提供了大量的安全保障。经过一些调优后，Rust Brotli实现尽管是100%安全的、数组范围检查的代码，但仍然比c实现的 Brotli 快"- Daniel Reiter Horn, Dropbox_
...
对于Go和Rust来说，编写一个可以在多个不同的操作系统和架构上运行的软件是比较容易的。编写一次，在任何地方编译。此外，Go和Rust都天生支持交叉编译，消除了通常与旧编译语言相关联的构建环境。
...
Go在开箱即用的时候就有很好的表现。从设计上看，Go 中没有提供可以让你获得更多性能方式。Rust的目标是使你能够从代码中挤出给你更多的性能; 在这方面，今天你真的找不到比Rust更快的语言了。然而，Rust 增加复杂性作为提升性能的代价。
...
"值得注意的是，使用Rust编写软件时，我们只需要做基本的优化。即使只进行了最基本的优化，即使只是做较小的优化, 使用 Rust 编写的软件在性能方面还是超越了使用 Go 大规模重构的版本。这是一个巨大的证明，与Go相比，使用Rust编写高效的程序是多么容易。" — [Jesse Howarth, Discord](https://blog.discord.com/why-discord-is-switching-from-go-to-rust-a190bbca2b1f)._
...
"Dropbox的工程发现，通过逐行移植Python代码到Go中，性能和延迟提高了5倍，内存使用通常比Python显著下降，因为没有GIL，进程数可能会减少。然而，当我们受到内存限制时，比如在桌面客户端软件或某些服务器进程中，我们会转向Rust，因为Rust中的手动内存管理比Go GC更高效。" — Daniel Reiter Horn, Dropbox_
...
Go的快速迭代的优势允许开发人员快速尝试想法，并在解决手头任务的工作代码中得到磨练。一般来说，这已经足够了，开发人员可以自由地转移到其他任务上。另一方面，与Go相比，Rust的编译时间更长，导致迭代时间更慢。这使得Go在更快的周转时间允许开发人员适应不断变化的需求的情况下工作得更好，而Rust在更多的时间可以用来制作更精细和性能更好的实现的情况下工作得更好。
...
Go类型系统的聪明之处在于，调用者可以定义接口，允许库返回扩展的结构，但需要狭窄的接口。Rust类型系统的聪明之处是匹配语法与Result<>的结合，在这里您可以静态地确定每个可能性都得到了处理，并且永远不必创建空值来满足未使用的返回参数。— Daniel Reiter Horn, Dropbox_
...
简单地说，真的没有比 Go 更平易近人的语言。许多团队几个星期就能够适应 Go 并且将Go编写的程序用于生产。此外，Go在语言中相对独一无二，因为它的语言设计和实践在它10多年的生命周期中是相当一致的。所以学习 Go 语言是值得的。相比之下，由于其复杂性而被认为是一种难以学习的语言。它通常需要几个月的学习并且适应 Rust，但这种额外的复杂性带来了精准控制和更高的性能。
...
"让Go不同于其他编程语言的是认知负荷。您可以用更少的代码做更多的事情，这使得您更容易对最终编写的代码进行推理和理解。大多数的Go代码最终看起来非常相似，所以，即使你正在使用一个全新的代码库，你也可以很快上手并运行。"- 美国运通忠诚平台Glen Balliet工程总监[American Express Uses Go for Payments & Rewards](https://go.dev/solutions/americanexpress/)_
...
也许Rust的最大优势之一是开发者对在内存管理、机器可用资源的使用、代码优化以及问题解决方案的设计等方面拥有大量的控制权。与Go相比，这并不是没有很大的复杂性成本，而Go的设计更适合于更快的探索时间和周转时间。
...
Rust的优势是最接近金属的最有利的。Go的优势是他们最有利的更近的用户。这并不是说要么不能在另一个空间中工作，但它会增加摩擦力。随着您的需求从灵活性转变为效率，它会更加强大的情况来重写 Rust 库。
