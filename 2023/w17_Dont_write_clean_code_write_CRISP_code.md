...
我不认为这是完全正确的词：我更喜欢“传统”的说法，但是这与 CRISP 的缩写并不符合。不过，当人们说“某某是惯用法”时，他们真正的意思是“这是传统的做法。”

传统做法是有用的：例如，有很多可能的方式来布置汽车的驾驶控制装置，我们习惯的那种方式并没有明显的最优解。它只是我们习惯的方式。没有法律阻止汽车制造商以不同的方式安排踏板，但实际上他们并没有这样做。即使存在某些*人体工程学*上的好处，这也不值得给用户带来的*认知*成本。
...
简单的一个方面是*直接性*：它做了罐子上说的事情。它没有奇怪和意外的副作用，也没有混淆几个不相关的事情。直接性与简短相反：不要写一个短而复杂的函数，而是写三个简单但相似的函数。
...
- 定义术语可以避免不必要的误解，有些常用的词汇如“简洁”、“可读性”等意义不明确，容易产生歧义。
- 测试只能证明代码符合测试人员的预期，很多时候甚至无法证明。
- 所有软件都有 Bug，分为已经发现和尚未发现的两类。
- “可读性”是所有人都赞成的，但是对于什么是“可读”的意见很不一致。
- 编写简洁易读的程序需要不断发现和消除代码中的阅读障碍。
- 程序的原创性并不重要，重要的是它是否正确、易读、易维护。
- 不要过于追求优化，因为它通常是不必要的，甚至有时是适得其反的。
- 时间是无限的，但空间是有限的，应该在速度和内存占用之间做出权衡。
- 在代码中去除不必要的抽象和重复可以提高可读性和可维护性。
- 写程序要先实现功能，然后再考虑简化。
- 遵循编程语言的特性和风格可以更好地编写高质量的代码。
- 读更多的代码可以提高编程技能。
- 简单并不意味着容易。
