...
Go 团队一直在密切关注并改进 Go 语言的垃圾回收器。从每50毫秒一次的10毫秒的 STW 暂停到每个 GC 的两个500微秒的STW暂停，整个改进过程可以在[这里](https://blog.golang.org/ismmkeynote)找到。

长期从事 Go 开发，我一直对其性能感到畏惧，因此我决定深入了解其中的机制，比如 Go 语言为什么如此高效和充满前途，了解它使用的是什么样的垃圾回收器，goroutine 如何在 OS 线程上多路复用，如何对 Go 程序进行性能分析，Go 运行时是如何工作的等等。在这篇文章中，我们将探讨 Go 的垃圾回收器的内容和方法。

在浏览互联网时，我发现了很多关于Go语言垃圾回收器的赞誉，由于我只有一个关于垃圾回收器是什么和如何工作的抽象思想，因此我开始阅读和发现，并在[这里](https://agrim123.github.io/posts/garbage-collection.html)记录了一些关于垃圾回收的笔记。

这篇博客仅仅是我在阅读几篇关于 Go 的垃圾回收器及其演变历程的博客后整理出的一些想法和结论的随笔。
...
Go 是一种以 C 类系统语言为传统的值导向语言，而不是以大多数托管运行时语言为传统的引用导向语言。值导向还有助于外部函数接口。这可能是 Go 与其他 GC 语言不同的最重要因素。

Go 是一种内存管理语言，这意味着大多数时候您不必担心手动内存管理，因为运行时会为您完成大量工作。然而，动态内存分配并不免费，程序的分配模式可以显着影响其性能：

> Go 二进制文件包含整个运行时。没有[即时编译](https://en.wikipedia.org/wiki/Just-in-time_compilation)。
...
Go 现在的 GC 不是 **generational** （一种垃圾回收算法）的。它只在后台运行一个普通的标记/清除。这有一些缺点：
...
在收集完成后，还有一个活动称为清除。清除是指回收未标记为使用的堆内存中的值的内存。 当应用程序 Goroutine 尝试分配堆内存中的新值时，该活动会发生。 清扫的延迟是对在堆内存中执行分配的额外代价，并与垃圾回收任何相关的延迟无关。

![GC Algorithm Phases](https://agrim123.github.io/images/GC%20Algorithm%20Phases.png)

## 如何让运行时知道什么时候开始回收垃圾？

收集器有一个节奏算法，用于确定何时开始收集。节奏的建模类似于一个控制问题，它试图找到启动 GC 周期的正确时间，以达到目标堆大小目标。Go 的默认节奏器将尝试在堆大小加倍时触发 GC 周期。它通过在当前 GC 周期的标记终止阶段设置下一个堆触发大小来实现这一点。因此，在标记所有活动内存后，它可以决定在当前活动集的总堆大小是目前活动集的2倍时触发下一个 GC。2倍的值来自运行时使用的变量`GOGC`，用于设置触发比率。
...
这调整了您想使用的 CPU 的数量以及您想使用的内存的数量。默认值为 100，这意味着堆的一半被用于存活内存，另一半用于分配。这可以向任一方向修改。
...
这就是 Go 的垃圾回收器的概述。当然，这不包括一切，我可能遗漏了一些要点，但我试图总结我所理解的一切。下面是我所遇到的一些非常好的参考资料，请一定要看一看！
