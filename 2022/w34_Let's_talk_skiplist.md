...
这个图本质上代表了按键的顺序组织的链表数据结构的集合。其中有不同的层级，它们有序的包含前一级别的元素子集。通常相同节点没有多个副本，它只是一种在不同级别表示相同节点的方法，但这可能取决于具体实现。例如在图中，存在于所有层级上的 `Node 5` 连接到了 `Node 24、Node 15、Node 7`。它可能看起来有多个副本，但它是同一个节点，只是具有不同级别的多个连接。
